...
export default function RegionDetails({ region }: { region: RegionDetailsDTO }) {
  return (
    <div className="region-details-container">
      ...
      <h3>Region Stats:</h3>
      <p><strong>Avg temperature:</strong> {region.avg_temperature.toFixed(2)} °C</p>
      <p><strong>Rating:</strong> {region.rating}</p>
      <p><strong>Avg potential:</strong> {region.avg_potential.toFixed(2)}</p>
      <p><strong>Infrastructure rating:</strong> {region.infrastructure_rating.toFixed(2)}</p>
      <p><strong>Index average:</strong> {region.index_average.toFixed(2)}</p>
      <p><strong>Best zone:</strong> {region.max_potential_zone}</p>
